...
private:
	...
	{
		Initialize = 0xFC8A,
		SetIoCapability = 0xFC85,
		SetAuthenticationRequirement = 0xFC86,
		ConfigureWhitelist = 0xFC92,
		SetNonDiscoverable = 0xFC81,
		SetDiscoverable = 0xFC83,
		UpdateAdvertiseData = 0xFC8E,
		PassKeyResp = 0xFC88,
		AllowRebond = 0xFC95,
		NumericComparisonValueConfirm = 0xFCA5
	};
	struct InitCommand
	{
		Role role;
		bool privacy;
		byte deviceNameCharLen;
	} __attribute__((packed));
	struct InitResponse
	{
		HCI::Status status;
		ushort serviceHandle;
		ushort devNameCharHandle;
		ushort appearanceCharHandle;
	} __attribute__((packed));
	struct SetDiscoverable
	{
		struct Com0
		{
			AdvertiseType Advertising_Type;
			...
			byte Local_Name[BLE_CMD_MAX_PARAM_LEN - 8];
		} __attribute__((packed));
		struct Com1
		{
			byte Service_Uuid_length;
			byte Service_Uuid_List[BLE_CMD_MAX_PARAM_LEN - 1];
		} __attribute__((packed));
		struct Com2
		{
	...
	struct UpdateAdvertiseData
	{
		struct Com0
		{
			byte AdvDataLen;
			byte AdvData[BLE_CMD_MAX_PARAM_LEN - 1];
		} __attribute__((packed));		
	};
	struct PassKeyResp
	{
	...
	struct NumericComparisonValueConfirm
	{
		struct Com0
		{
			ushort handle;
...
public:
	...
	struct SetIoCapability
	{
		byte capability;
	} __attribute__((packed));
	struct SetAuthenticationRequirement
	{
		byte bondingMode;
		byte mitmMode;
		byte SCsupport;
		byte keyPressNotificationSupport;
		byte minEncryptionKeySize;
		byte maxEncryptionKeySize;
		byte useFixedPin;
		uint fixedPin;
...
/**
	...
	*/
	static HCI::Status setNonDiscoverable();
...
//	                    * @retval Value indicating success or error code.
//	                    */
//	static HCI::Status aci_gap_set_limited_discoverable(	byte Advertising_Type, ushort Advertising_Interval_Min, ushort Advertising_Interval_Max, byte Own_Address_Type,
//														byte Advertising_Filter_Policy, byte Local_Name_Length, byte Local_Name[], byte Service_Uuid_length,
//														byte Service_Uuid_List[],ushort Slave_Conn_Interval_Min, ushort Slave_Conn_Interval_Max);
	                ...
	                connIntervalmin = Slave_Conn_Interval_Min x 1.25ms.
	                  ...
	                  * @param Slave_Conn_Interval_Max Slave connection interval maximum value suggested by Peripheral.
	                  ...
	                  connIntervalmax = Slave_Conn_Interval_Max x 1.25ms
	                    ...
	                    * @retval Value indicating success or error code.
	                    */
	static HCI::Status setDiscoverable(	AdvertiseType Advertising_Type, ushort Advertising_Interval_Min, ushort Advertising_Interval_Max, byte Own_Address_Type,
												byte Advertising_Filter_Policy, byte Local_Name_Length, byte Local_Name[], byte Service_Uuid_length, byte Service_Uuid_List[],
												ushort Slave_Conn_Interval_Min, ushort Slave_Conn_Interval_Max);
...
//	        * @retval Value indicating success or error code.
//	        */
//	static HCI::Status aci_gap_set_direct_connectable(byte Own_Address_Type, byte Directed_Advertising_Type, byte Direct_Address_Type, byte Direct_Address[6], ushort Advertising_Interval_Min, ushort Advertising_Interval_Max);
	...
	/**
	  * @brief Set the IO capabilities of the device. This command has to be given only when the device is
	  not in a connected state.
	    ...
	    * @retval Value indicating success or error code.
	    */
	static HCI::Status setIoCapability(byte capability);
	      ...
	      notified
	        ...
	        * @param Fixed_Pin Fixed pin to be used during pairing if MIMT protection is enabled.
	        Any random value between 0 to 999999
	          ...
	          * @retval Value indicating success or error code.
	          */
	static HCI::Status setAuthenticationRequirement(byte bondingMode, byte mitmMode, byte SCsupport, byte keyPressNotificationSupport, byte minEncryptionKeySize, byte maxEncryptionKeySize, byte useFixedPin, uint fixedPin, byte identityAddressType);
...
//	      * @retval Value indicating success or error code.
//	      */
//	static HCI::Status aci_gap_set_authorization_requirement(ushort Connection_Handle, byte Authorization_Enable);
	  ...
	  event. The command parameter contains the pass key which will be used during the pairing process.
	    ...
	    * @param Pass_Key Pass key that will be used during the pairing process. 
	    Must be a six-digit decimal number.
	      ...
	      * @retval Value indicating success or error code.
	      */
	static HCI::Status passKeyResp(ushort handle, uint key);
...
//	    * @retval Value indicating success or error code.
//	    */
//	static HCI::Status aci_gap_authorization_resp(ushort Connection_Handle, byte Authorize);
	...
	/**
	  * @brief Initialize the GAP layer. Register the GAP service with the GATT.
	  ...
	  Note that if the Peripheral Preferred Connection Parameters characteristic is added, its handle is equal to the Appearance characteristic handle plus 2.
	    ...
	    * @retval Value indicating success or error code.
	    */
	static HCI::Status init(Role role, byte privacy, byte deviceNameCharLength, InitHandlers & handlers);
...
//	    * @retval Value indicating success or error code.
//	    */
//	static HCI::Status aci_gap_set_non_connectable(byte Advertising_Event_Type, byte Own_Address_Type);
//
//	/**
...
//	        * @retval Value indicating success or error code.
//	        */
//	static HCI::Status aci_gap_set_undirected_connectable(ushort Advertising_Interval_Min, ushort Advertising_Interval_Max, byte Own_Address_Type, byte Adv_Filter_Policy);
//
//	/**
...
//	    * @retval Value indicating success or error code.
//	    */
//	static HCI::Status aci_gap_slave_security_req(ushort Connection_Handle);
	...
	/**
	  * @brief This command can be used to update the advertising data for a particular AD type. If the AD
	  ...
	  rejected and the old data is retained.
	    ...
	    * @retval Value indicating success or error code.
	    */
	static HCI::Status updateAdvData(byte AdvDataLen, byte AdvData[]);
...
//	    * @retval Value indicating success or error code.
//	    */
//	static HCI::Status aci_gap_delete_ad_type(byte ADType);
//
//	/**
...
//	  * @retval Value indicating success or error code.
//	  */
//	static HCI::Status aci_gap_get_security_level(ushort Connection_Handle, byte *Security_Mode, byte *Security_Level);
//
//	/**
...
//	  * @retval Value indicating success or error code.
//	  */
//	static HCI::Status aci_gap_set_event_mask(ushort GAP_Evt_Mask);
	...
	/**
	  * @brief Add addresses of bonded devices into the controller's whitelist.
	  The command returns an error if it was unable to add the bonded devices into the whitelist.
	    * @retval Value indicating success or error code.
	    */
	static HCI::Status configureWhitelist();
...
//	    * @retval Value indicating success or error code.
//	    */
//	static HCI::Status aci_gap_terminate(ushort Connection_Handle, byte Reason);
//
//	/**
//	  * @brief Clear the security database. All the devices in the security database will be removed.
//	  * @retval Value indicating success or error code.
//	  */
//	static HCI::Status aci_gap_clear_security_db();
	...
	/**
	  * @brief Allows the security manager to complete the pairing procedure and re-bond with the master.
	  ...
	  command is not given on receiving the event, the bonding procedure will timeout.
	    ...
	    * @retval Value indicating success or error code.
	    */
	static HCI::Status allowRebond(ushort handle);
...
//	        * @retval Value indicating success or error code.
//	        */
//	static HCI::Status aci_gap_start_limited_discovery_proc(ushort LE_Scan_Interval, ushort LE_Scan_Window, byte Own_Address_Type, byte Filter_Duplicates);
//
//	/**
...
//	        * @retval Value indicating success or error code.
//	        */
//	static HCI::Status aci_gap_start_general_discovery_proc(ushort LE_Scan_Interval, ushort LE_Scan_Window, byte Own_Address_Type, byte Filter_Duplicates);
//
//	/**
...
//	                    * @retval Value indicating success or error code.
//	                    */
//	static HCI::Status aci_gap_start_name_discovery_proc(	ushort LE_Scan_Interval, ushort LE_Scan_Window, byte Peer_Address_Type, byte Peer_Address[6],
//														byte Own_Address_Type, ushort Conn_Interval_Min, ushort Conn_Interval_Max, ushort Conn_Latency,
//														ushort Supervision_Timeout, ushort Minimum_CE_Length, ushort Maximum_CE_Length);
//
//	/**
...
//	                  * @retval Value indicating success or error code.
//	                  */
//	static HCI::Status aci_gap_start_auto_connection_establish_proc(	ushort LE_Scan_Interval, ushort LE_Scan_Window, byte Own_Address_Type, ushort Conn_Interval_Min,
//																	ushort Conn_Interval_Max, ushort Conn_Latency, ushort Supervision_Timeout, ushort Minimum_CE_Length,
//																	ushort Maximum_CE_Length, byte Num_of_Whitelist_Entries, Whitelist_Entry_t Whitelist_Entry[]);
//
//	/**
...
//	        * @retval Value indicating success or error code.
//	        */
//	static HCI::Status aci_gap_start_general_connection_establish_proc(byte LE_Scan_Type, ushort LE_Scan_Interval, ushort LE_Scan_Window, byte Own_Address_Type, byte Scanning_Filter_Policy, byte Filter_Duplicates);
//
//	/**
...
//	        * @retval Value indicating success or error code.
//	        */
//	static HCI::Status aci_gap_start_selective_connection_establish_proc(byte LE_Scan_Type, ushort LE_Scan_Interval, ushort LE_Scan_Window, byte Own_Address_Type, byte Scanning_Filter_Policy, byte Filter_Duplicates, byte Num_of_Whitelist_Entries, Whitelist_Entry_t Whitelist_Entry[]);
//
//	/**
...
//	                    * @retval Value indicating success or error code.
//	                    */
//	static HCI::Status aci_gap_create_connection(	ushort LE_Scan_Interval, ushort LE_Scan_Window, byte Peer_Address_Type, byte Peer_Address[6],
//												byte Own_Address_Type, ushort Conn_Interval_Min, ushort Conn_Interval_Max, ushort Conn_Latency,
//												ushort Supervision_Timeout, ushort Minimum_CE_Length, ushort Maximum_CE_Length);
//
//	/**
...
//	    * @retval Value indicating success or error code.
//	    */
//	static HCI::Status aci_gap_terminate_gap_proc(byte Procedure_Code);
//
//	/**
...
//	              * @retval Value indicating success or error code.
//	              */
//	static HCI::Status aci_gap_start_connection_update(ushort Connection_Handle, ushort Conn_Interval_Min, ushort Conn_Interval_Max, ushort Conn_Latency, ushort Supervision_Timeout, ushort Minimum_CE_Length, ushort Maximum_CE_Length);
//
//	/**
...
//	    * @retval Value indicating success or error code.
//	    */
//	static HCI::Status aci_gap_send_pairing_req(ushort Connection_Handle, byte Force_Rebond);
//
//	/**
...
//	    * @retval Value indicating success or error code.
//	    */
//	static HCI::Status aci_gap_resolve_private_addr(byte Address[6], byte Actual_Address[6]);
//
//	/**
...
//	          * @retval Value indicating success or error code.
//	          */
//	static HCI::Status aci_gap_set_broadcast_mode(ushort Advertising_Interval_Min, ushort Advertising_Interval_Max, byte Advertising_Type, byte Own_Address_Type, byte Adv_Data_Length, byte Adv_Data[], byte Num_of_Whitelist_Entries, Whitelist_Entry_t Whitelist_Entry[]);
//
//	/**
...
//	        * @retval Value indicating success or error code.
//	        */
//	static HCI::Status aci_gap_start_observation_proc(ushort LE_Scan_Interval, ushort LE_Scan_Window, byte LE_Scan_Type, byte Own_Address_Type, byte Filter_Duplicates, byte Scanning_Filter_Policy);
//
//	/**
...
//	    * @retval Value indicating success or error code.
//	    */
//	static HCI::Status aci_gap_get_bonded_devices(byte *Num_of_Addresses, Bonded_Device_Entry_t Bonded_Device_Entry[]);
//
//	/**
...
//	    * @retval Value indicating success or error code.
//	    */
//	static HCI::Status aci_gap_is_device_bonded(byte Peer_Address_Type, byte Peer_Address[6]);
	...
	/**
	  ...
	  * @param Confirm_Yes_No 0 : The Numeric Values showed on both local and peer device are different!
	  1 : The Numeric Values showed on both local and peer device are equal!
	    ...
	    * @retval Value indicating success or error code.
	    */
	static HCI::Status numericComparisonValueConfirm(ushort handle, bool confirm);
...
//	  * @retval Value indicating success or error code.
//	  */
//	static HCI::Status aci_gap_passkey_input(ushort Connection_Handle, byte Input_Type);
//
//	/**
...
//	  * @retval Value indicating success or error code.
//	  */
//	static HCI::Status aci_gap_get_oob_data(byte OOB_Data_Type, byte *Address_Type, byte Address[6], byte *OOB_Data_Len, byte OOB_Data[16]);
//
//	/**
...
//	  * @retval Value indicating success or error code.
//	  */
//	static HCI::Status aci_gap_set_oob_data(byte Device_Type, byte Address_Type, byte Address[6], byte OOB_Data_Type, byte OOB_Data_Len, byte OOB_Data[16]);
//
//	/**
...
//	  * @retval Value indicating success or error code.
//	  */
//	static HCI::Status aci_gap_add_devices_to_resolving_list(byte Num_of_Resolving_list_Entries, Whitelist_Identity_Entry_t Whitelist_Identity_Entry[], byte Clear_Resolving_List);
//
//	/**
...
//	  * @retval Value indicating success or error code.
//	  */
//	static HCI::Status aci_gap_remove_bonded_device(byte Peer_Identity_Address_Type, byte Peer_Identity_Address[6]);
};